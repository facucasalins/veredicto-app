...
// {nivel, fuente: override|regla|claude|nd, motivador, motivadorTipo, confianza, razon?}.
// `cache:false` en el resultado avisa que sin Upstash la clasificación por Claude se repite.
export async function classifyRows(rows, tab) {
  ...
  // Claude en tandas de 20 (con 40 y razonamiento por creativo la salida pasaba max_tokens); lo que
  // falle o falte queda "nd" y se reintenta en la próxima carga
  const TANDA = 20;
  for (let i = 0; i < pendientes.length; i += TANDA) {
    const tanda = pendientes.slice(i, i + TANDA);
    let res = {};
    try { res = await claudeBatch(tanda); } catch (e) { console.error("[conciencia] claude:", e.message); }
    for (const r of tanda) {
      const c = res[r.fingerprint];
      if (!c) { out[r.fingerprint] = { nivel: null, fuente: "nd", motivador: "", motivadorTipo: "", confianza: "baja" }; continue; }
      out[r.fingerprint] = { ...c, fuente: "claude" };
      _mem[keyNivel(tab, r.fingerprint)] = c;
      if (upstash) kvSet(keyNivel(tab, r.fingerprint), c).catch(() => {});
    }
  }
  return { niveles: out, cache: upstash, claude: !!process.env.ANTHROPIC_API_KEY };
}
