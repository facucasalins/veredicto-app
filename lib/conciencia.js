...
export async function classifyRows(rows, tab, { maxBatches = 2 } = {}) {
  ...
  rows.forEach((r, i) => {
    ...
    if (ov && ov.nivel) { out[fp] = { nivel: ov.nivel, fuente: "override", motivador: ov.motivador || "", motivadorTipo: ov.motivadorTipo || "", confianza: "alta", nivel_previo: ov.nivel_previo ?? null, fuente_previa: ov.fuente_previa || null, t: ov.t || null }; return; }
    const rg = reglas(r);
