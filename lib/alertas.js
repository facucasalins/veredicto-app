...
// Corre todos los chequeos y devuelve la lista de alertas [{ nivel, cuenta, titulo, detalle, id?, store? }].
// nivel: "critico" | "aviso". `id` = cuenta de ads (act_/g:) y `store` = tienda/propiedad — el front
// las usa para mostrar SOLO las alertas de la cuenta seleccionada; sin id ni store la alerta es
// global (token caído) y se ve siempre. Cada bloque tiene su catch: un chequeo caído no tira el resto.
export async function runChecks() {
  ...
  for (const a of metaAccs) {
    if (a.status !== 1 && a.status !== undefined) {
      alertas.push({ nivel: "critico", id: a.id, cuenta: a.name.replace(/^⚠ /, ""), titulo: "Cuenta de Meta con problema", detalle: a.name + " — Meta puede estar frenando la entrega de anuncios." });
    }
  }
  ...
  await Promise.all(cuentas.map(async (c) => {
    try {
      ...
      if (deAyer.spend === 0 && prom > 1000) {
        alertas.push({ nivel: "critico", id: c.id, cuenta: c.name, titulo: "Sin inversión ayer", detalle: `Ayer gastó $0 cuando venía invirtiendo ~$${Math.round(prom).toLocaleString("es-AR")}/día. ¿Campañas apagadas, budget agotado o problema de pago?` });
      }
      ...
      if (hoy7.spend > 5000 && prev7.spend > 5000 && prev7.roasMeta > 0) {
        ...
        if (caida > 0.3) {
          alertas.push({ nivel: "aviso", id: c.id, cuenta: c.name, titulo: "ROAS cayendo fuerte", detalle: `ROAS ${hoy7.roasMeta.toFixed(1)}x en los últimos 7 días vs ${prev7.roasMeta.toFixed(1)}x la semana anterior (−${Math.round(caida * 100)}%).` });
        }
      }
    ...
    await Promise.all(props.map(async (p) => {
      try {
        ...
        if (cur.s > 500 && prev.s > 500 && prev.c / prev.s > 0) {
          ...
          if (caida > 0.3) {
            alertas.push({ nivel: "aviso", id: p.account || null, store: p.store || null, cuenta: p.name, titulo: "Conversión del sitio cayendo", detalle: `CR ${(crCur * 100).toFixed(2)}% en los últimos 7 días vs ${(crPrev * 100).toFixed(2)}% la semana anterior (−${Math.round(caida * 100)}%). Si el tráfico se mantuvo, el problema es el SITIO (checkout, stock, precios), no la pauta.` });
          }
        }
...
// ─── Salud del cron ───────────────────────────────────────────────────────────────────────────
// Cada intento del cron (bien o mal) deja rastro en Upstash: si la banda dice "chequeo 27/7" y hoy
// es 10/9, acá se ve si el cron llegó y falló (CRON_SECRET, error) o directamente no llegó.
export async function registrarCorrida(info) {
  if (!storeEnabled()) return;
  await kvSet("nusa:alertas:cron", { t: new Date().toISOString(), ...info }).catch(() => {});
}
export async function leerCorrida() {
  if (!storeEnabled()) return null;
  return kvGet("nusa:alertas:cron").catch(() => null);
}
// Alerta sintética (global) si el último chequeo tiene más de 48 h. `ultimo` = ISO del último
// chequeo guardado; `corrida` = último intento del cron (puede haber fallado).
export function alertaCronCaido(ultimo, corrida) {
  const horas = ultimo ? (Date.now() - new Date(ultimo).getTime()) / 36e5 : Infinity;
  if (horas < 48) return null;
  const secret = !!process.env.CRON_SECRET;
  const causa = !secret
    ? "Falta CRON_SECRET en Vercel: el cron llega sin credencial y la ruta lo rechaza (401). Cargala en Settings → Environment Variables y redeployá."
    : corrida && corrida.ok === false ? `El último intento del cron falló: ${corrida.error || "sin detalle"}.`
    : corrida ? `El cron corrió por última vez ${new Date(corrida.t).toLocaleString("es-AR")} y desde entonces no volvió a llegar: revisá Vercel → Settings → Cron Jobs (tiene que estar habilitado en el deploy de producción).`
    : "No hay registro de ningún intento del cron: revisá que el deploy de producción tenga el cron habilitado (Vercel → Settings → Cron Jobs) y que CRON_SECRET esté cargada.";
  return { nivel: "critico", cuenta: "NUSA APP", titulo: `Cron de alertas sin correr hace ${isFinite(horas) ? Math.round(horas) + " h" : "nunca"}`, detalle: causa };
}
