...
// `ttlSec` (opcional) → la clave expira sola (EX): para caches, no para historiales.
export async function kvSet(key, value, ttlSec = null) {
  const c = cfg();
  if (!c) throw new Error("Storage no configurado");
  const ex = ttlSec && ttlSec > 0 ? `?EX=${Math.round(ttlSec)}` : "";
  const res = await fetch(`${c.url}/SET/${encodeURIComponent(key)}${ex}`, {
    method: "POST",
    headers: { Authorization: `Bearer ${c.token}` },
...
export async function kvDel(key) {
  const c = cfg();
  if (!c) throw new Error("Storage no configurado");
  const res = await fetch(`${c.url}/DEL/${encodeURIComponent(key)}`, { method: "POST", headers: { Authorization: `Bearer ${c.token}` }, cache: "no-store" });
  const j = await res.json();
  if (!res.ok || j.error) throw new Error(j.error || `Upstash ${res.status}`);
  return j.result;
}

// MGET de varias claves en UN request (comando por POST body). Devuelve array alineado con `keys`
// (null donde no existe o no parsea). Para leer N clasificaciones/overrides de una.
export async function kvMget(keys) {
  const c = cfg();
  if (!c) throw new Error("Storage no configurado");
  if (!keys.length) return [];
  const res = await fetch(c.url, { method: "POST", headers: { Authorization: `Bearer ${c.token}`, "Content-Type": "application/json" }, body: JSON.stringify(["MGET", ...keys]), cache: "no-store" });
  const j = await res.json();
  if (!res.ok || j.error) throw new Error(j.error || `Upstash ${res.status}`);
  return (j.result || []).map((v) => { if (v == null) return null; try { return JSON.parse(v); } catch { return null; } });
}
