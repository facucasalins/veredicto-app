...
export function fechasAR(now = new Date()) {
  const hoyStr = new Intl.DateTimeFormat("en-CA", { timeZone: TZ, year: "numeric", month: "2-digit", day: "2-digit" }).format(now);
  const hoy = new Date(hoyStr + "T00:00:00Z"); // medianoche del día argentino, operado en UTC
