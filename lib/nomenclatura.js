...
export function buildRows(ads) {
  const m = {};
  for (const a of ads) {
    const p = parseName(a.name);
    const k = p.fingerprint;
    if (!m[k]) m[k] = { id: k, nombre: label(p), ang: p.angulo, sec: "—", split: "100/0", hook: p.concepto, fmt: p.formato, spend: 0, revenue: 0, ventas: 0, placements: 0, tipo: p.tipo, audSpend: {}, bk: {} };
    const g = m[k];
    g.spend += a.spend; g.revenue += a.spend * a.roas; g.ventas += a.ventas; g.placements += 1;
    const aud = parseAudience(a.adset);
    g.audSpend[aud] = (g.audSpend[aud] || 0) + a.spend;
    // desglose por campaña + adset (para el acordeón de composición del ranking)
    const camp = a.campaign || "—", adset = a.adset || "—", bk = camp + " ‖ " + adset;
    if (!g.bk[bk]) g.bk[bk] = { campaign: camp, adset, spend: 0, revenue: 0, ventas: 0 };
  ...
  return Object.values(m).map((g) => {
    ...
    const breakdown = Object.values(g.bk)
      .map((b) => ({ campaign: b.campaign, adset: b.adset, spend: Math.round(b.spend), ventas: Math.round(b.ventas), roas: b.spend ? +(b.revenue / b.spend).toFixed(2) : 0 }))
      .sort((a, b) => b.spend - a.spend);
    return { id: g.id, nombre: g.nombre, ang: g.ang, sec: g.sec, split: g.split, aud, hook: g.hook, fmt: g.fmt, spend: Math.round(g.spend), roas: +roas.toFixed(2), ventas: Math.round(g.ventas), cpa: Math.round(cpa), placements: g.placements, breakdown };
  }).sort((a, b) => b.spend - a.spend);
}

// Agrega a nivel ANUNCIO por audiencia (el mismo creativo corre en varias audiencias).
export function buildAudienceRows(ads) {
  const m = {};
  for (const a of ads) {
    const key = parseAudience(a.adset);
