...
export function buildRows(ads, audMap, statusMap, tipoMap) {
  ...
  for (const a of ads) {
    ...
    if (!g.bk[bk]) g.bk[bk] = { campaign: camp, adset, aud, spend: 0, revenue: 0, ventas: 0, qCal: {} };
  ...
  return Object.values(m).map((g) => {
    ...
    const breakdown = Object.values(g.bk)
      .map((b) => ({ campaign: b.campaign, adset: b.adset, aud: b.aud, spend: Math.round(b.spend), ventas: Math.round(b.ventas), roas: b.spend ? +(b.revenue / b.spend).toFixed(2) : 0, calidad: domRank(b.qCal) }))
