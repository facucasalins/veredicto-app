...
export function buildRows(ads) {
  ...
  for (const a of ads) {
    ...
    if (!m[k]) m[k] = { id: k, nombre: label(p), ang: p.angulo, sec: "—", split: "100/0", hook: p.concepto, fmt: p.formato, spend: 0, revenue: 0, ventas: 0, placements: 0, tipo: p.tipo, audSpend: {} };
    const g = m[k];
  ...
  return Object.values(m).map((g) => {
    ...
    return { id: g.id, nombre: g.nombre, ang: g.ang, sec: g.sec, split: g.split, aud, hook: g.hook, fmt: g.fmt, spend: Math.round(g.spend), roas: +roas.toFixed(2), ventas: Math.round(g.ventas), cpa: Math.round(cpa), placements: g.placements };
