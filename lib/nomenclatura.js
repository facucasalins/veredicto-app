...
export function buildRows(ads, audMap, statusMap, tipoMap) {
  const m = {};
  for (const a of ads) {
    const p = parseName(a.name);
    const k = p.fingerprint;
    if (!m[k]) m[k] = { id: k, nombre: label(p), ang: p.angulo, sec: "—", split: "100/0", hook: p.concepto, fmt: p.formato, spend: 0, revenue: 0, ventas: 0, conversaciones: 0, placements: 0, activos: 0, audSpend: {}, tipoSpend: {}, bk: {}, qCal: {}, qInt: {}, qConv: {}, imp: 0, video3s: 0, clics: 0, lpv: 0, vc: 0, atc: 0, ic: 0, freqW: 0 };
    const g = m[k];
