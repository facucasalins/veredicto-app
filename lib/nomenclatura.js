...
export function buildRows(ads, audMap, statusMap, tipoMap) {
  ...
  for (const a of ads) {
    ...
    if (!m[k]) m[k] = { id: k, nombre: label(p), ang: p.angulo, sec: "—", split: "100/0", hook: p.concepto, fmt: p.formato, spend: 0, revenue: 0, ventas: 0, conversaciones: 0, placements: 0, activos: 0, audSpend: {}, tipoSpend: {}, bk: {}, qCal: {}, qInt: {}, qConv: {} };
    const g = m[k];
    ...
    if (!g.bk[bk]) g.bk[bk] = { campaign: camp, adset, aud, spend: 0, revenue: 0, ventas: 0, conversaciones: 0, qCal: {} };
    g.bk[bk].spend += a.spend; g.bk[bk].revenue += a.spend * a.roas; g.bk[bk].ventas += a.ventas; g.bk[bk].conversaciones += (a.conversaciones || 0);
  ...
  return Object.values(m).map((g) => {
    ...
    const breakdown = Object.values(g.bk)
      .map((b) => ({ campaign: b.campaign, adset: b.adset, aud: b.aud, spend: Math.round(b.spend), ventas: Math.round(b.ventas), conversaciones: Math.round(b.conversaciones || 0), roas: b.spend ? +(b.revenue / b.spend).toFixed(2) : 0, calidad: domRank(b.qCal) }))
