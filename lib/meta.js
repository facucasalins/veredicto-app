...
export async function getAds(accountId, datePreset = "last_30d", range = null) {
  ...
  const fields = ["ad_id", "ad_name", "adset_id", "adset_name", "campaign_name", "spend", "purchase_roas", "actions", "impressions", "quality_ranking", "engagement_rate_ranking", "conversion_rate_ranking"].join(",");
  ...
  return data.map((d) => {
    ...
    return { id: d.ad_id, name: d.ad_name || "", adset_id: d.adset_id || "", adset: d.adset_name || "", campaign: d.campaign_name || "", spend: num(d.spend), roas, ventas: purchases, conversaciones, impressions: num(d.impressions), calidad: rk(d.quality_ranking), interaccion: rk(d.engagement_rate_ranking), conversion: rk(d.conversion_rate_ranking) };
  });
}
