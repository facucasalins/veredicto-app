...
export async function getAdPreviewUrl(accountId, adId) {
  const q = (p) => `${GRAPH}/${V}/${p}${p.includes("?") ? "&" : "?"}access_token=${encodeURIComponent(token())}`;
  ...
  for (const fmt of PREVIEW_FORMATS) {
    try {
      const pv = await (await fetch(q(`${adId}/previews?ad_format=${fmt}`), { cache: "no-store" })).json();
  ...
  try {
    const ad = await (await fetch(q(`${adId}?fields=creative{effective_object_story_id}`), { cache: "no-store" })).json();
