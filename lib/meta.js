...
export async function getAccountSpend(accountId, since, until) {
  ...
  // Sumamos tráfico/embudo: impresiones, clics al enlace y el array actions (de ahí salen compras y
  // también landing_page_view = "visitas a la web" desde los anuncios, add_to_cart, checkout, etc.).
  const fields = ["spend", "purchase_roas", "actions", "impressions", "inline_link_clicks"].join(",");
  ...
  // Suma el valor del primer action_type presente de la lista (cubre los alias omni/pixel de Meta).
  ...
  return {
    spend: Math.round(spend), roasMeta: roas, ventasMeta: Math.round(purchases),
    // tráfico PAGO de Meta (no es el total del sitio — eso sería GA4)
    impresiones: Math.round(num(row.impressions)),
    clicsEnlace: Math.round(num(row.inline_link_clicks)),
    visitasWeb: Math.round(act("landing_page_view", "omni_landing_page_view")), // gente que cargó el sitio
    verContenido: Math.round(act("view_content", "omni_view_content", "offsite_conversion.fb_pixel_view_content")),
    addToCart: Math.round(act("add_to_cart", "omni_add_to_cart", "offsite_conversion.fb_pixel_add_to_cart")),
    iniciarCheckout: Math.round(act("initiate_checkout", "omni_initiated_checkout", "offsite_conversion.fb_pixel_initiate_checkout")),
  };
}
