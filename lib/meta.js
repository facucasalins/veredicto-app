...
export async function getAccountSpendDaily(accountId, since, until) {
  const tr = encodeURIComponent(JSON.stringify({ since, until }));
  let url = `${GRAPH}/${V}/act_${accountId}/insights?level=account&fields=spend,actions,inline_link_clicks&time_increment=1&time_range=${tr}&limit=500&access_token=${encodeURIComponent(token())}`;
  const out = [];
...
// OJO: el edge /ads EXCLUYE los anuncios ARCHIVADOS por default. Un anuncio que corrió en el
// período y después se archivó tiene spend en insights pero no aparecía acá → quedaba "sin estado"
// (medido: 117 de 436 anuncios con spend en 30 días en una cuenta real, todos ARCHIVED). Por eso
// pedimos explícitamente todos los estados. DELETED no es un valor válido del filtro (400).
const ESTADOS_ADS = ["ACTIVE", "PAUSED", "ARCHIVED", "PENDING_REVIEW", "DISAPPROVED", "PREAPPROVED", "PENDING_BILLING_INFO", "CAMPAIGN_PAUSED", "ADSET_PAUSED", "IN_PROCESS", "WITH_ISSUES"];
export async function getAdStatuses(accountId) {
  ...
  let url = `${GRAPH}/${V}/act_${accountId}/ads?fields=${fields}&effective_status=${encodeURIComponent(JSON.stringify(ESTADOS_ADS))}&limit=500&access_token=${encodeURIComponent(token())}`;
  const map = {};
...
export async function getAds(accountId, datePreset = "last_30d", range = null) {
  ...
  const fields = ["ad_id", "ad_name", "adset_id", "adset_name", "campaign_id", "campaign_name", "spend", "purchase_roas", "actions", "impressions", "reach", "frequency", "inline_link_clicks", "video_thruplay_watched_actions", "video_p50_watched_actions", "video_p100_watched_actions", "video_avg_time_watched_actions", "quality_ranking", "engagement_rate_ranking", "conversion_rate_ranking"].join(",");
  ...
  return data.map((d) => {
    ...
    return {
      id: d.ad_id, name: d.ad_name || "", adset_id: d.adset_id || "", adset: d.adset_name || "", campaign_id: d.campaign_id || "", campaign: d.campaign_name || "",
      ...
      video3s: act("video_view"), thruplay: vidArr(d.video_thruplay_watched_actions), video50: vidArr(d.video_p50_watched_actions), video100: vidArr(d.video_p100_watched_actions),
      videoAvgTime: vidArr(d.video_avg_time_watched_actions), // segundos promedio vistos por reproducción
...
export async function getTrackingHealth(accountId) {
  ...
  try {
    let url2 = `${GRAPH}/${V}/act_${accountId}/adsets?fields=effective_status,promoted_object&limit=500&access_token=${encodeURIComponent(token())}`;
    for (let i = 0; i < 10 && url2; i++) {
      const rr = await fetch(url2, { cache: "no-store" });
...
export async function getAdPreviewUrl(accountId, adId) {
  ...
  try {
    const pv = await (await fetch(q(`${adId}/previews?ad_format=MOBILE_FEED_STANDARD`), { cache: "no-store" })).json();
    const body = pv.data && pv.data[0] && pv.data[0].body;
    const src = body && (body.match(/src="([^"]+)"/) || [])[1];
    if (src) return { url: src.replace(/&amp;/g, "&"), tipo: "preview" };
  } catch { /* probamos el fallback */ }
  try {
