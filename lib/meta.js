...
// effective_status ya contempla si el adset o la campaña están pausados. Sirve para no recomendar
// "pausá X" sobre algo que ya está pausado (el insight es histórico, el estado es actual).
export async function getAdStatuses(accountId) {
  let url = `${GRAPH}/${V}/act_${accountId}/ads?fields=id,effective_status&limit=500&access_token=${encodeURIComponent(token())}`;
  const map = {};
  for (let i = 0; i < 20 && url; i++) {
    const r = await fetch(url, { cache: "no-store" });
    const j = await r.json();
    if (j.error) throw new Error(j.error.message || "Error de Meta (ads status)");
    for (const a of j.data || []) map[a.id] = a.effective_status;
...
export async function getAdsetBudgets(accountId) {
  const fields = "id,name,daily_budget,lifetime_budget,effective_status,campaign{id,name,daily_budget,lifetime_budget}".replace(/{/g, "%7B").replace(/}/g, "%7D");
...
export async function getAds(accountId, datePreset = "last_30d", range = null) {
  ...
  return (j.data || []).map((d) => {
    ...
    return { id: d.ad_id, name: d.ad_name || "", adset_id: d.adset_id || "", adset: d.adset_name || "", campaign: d.campaign_name || "", spend: num(d.spend), roas, ventas: purchases, impressions: num(d.impressions) };
  });
}
