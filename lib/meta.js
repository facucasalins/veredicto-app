...
// Todas las llamadas a Graph pasan por acá: el token va en el header Authorization, NUNCA en la
// URL (una URL con access_token termina en logs, mensajes de error o paging.next).
const graphFetch = (url) => fetch(url, { cache: "no-store", headers: { Authorization: `Bearer ${token()}` } });

// Estados problemáticos de una cuenta (account_status de Meta) que NO deben esconderla del
// panel: se muestra con ⚠ y el motivo, así se ve de una que el problema es de Meta/facturación
// y no del panel (pasó con una cuenta UNSETTLED: desaparecía en silencio). Las cerradas
// (100/101) sí quedan afuera — son ruido, no un problema a resolver.
const STATUS_PROBLEMA = {
  2: "inhabilitada por Meta",
  3: "pago pendiente",
...
export async function getAccounts() {
  ...
  const url = `${GRAPH}/${V}/me/adaccounts?fields=${fields}&limit=200`;
  ...
  return (j.data || [])
    ...
    .filter((a) => a.status === 1 || a.status === undefined || STATUS_PROBLEMA[a.status])
    .map((a) => (STATUS_PROBLEMA[a.status] ? { ...a, name: `⚠ ${a.name} — ${STATUS_PROBLEMA[a.status]}` } : a));
}
...
export async function getAccountSpend(accountId, since, until) {
  ...
  const url = `${GRAPH}/${V}/act_${accountId}/insights?level=account&fields=${fields}&time_range=${tr}`;
  const r = await graphFetch(url);
  const j = await r.json();
  if (j.error) throw new Error(j.error.message || "Error de Meta");
  const row = (j.data || [])[0] || {};
...
// cuenta ("¿cuánto consume por día?") y el gráfico diario del Dashboard. Paginado.
// clics = inline_link_clicks; visitas = landing_page_view (carga real de la web) con fallback a
// clics si el pixel no reporta LPV — el gráfico cruza gasto/visitas/ventas por día.
export async function getAccountSpendDaily(accountId, since, until) {
  const tr = encodeURIComponent(JSON.stringify({ since, until }));
  let url = `${GRAPH}/${V}/act_${accountId}/insights?level=account&fields=spend,actions,inline_link_clicks&time_increment=1&time_range=${tr}&limit=500`;
  const out = [];
  for (let i = 0; i < 10 && url; i++) {
    ...
    for (const row of j.data || []) {
      ...
      if (Array.isArray(row.actions)) {
        ...
        const l = row.actions.find((a) => ["landing_page_view", "omni_landing_page_view"].includes(a.action_type));
        if (l) lpv = num(l.value);
      }
      const clics = num(row.inline_link_clicks);
      out.push({ fecha: row.date_start, spend: num(row.spend), ventas: Math.round(ventas), clics: Math.round(clics), visitas: Math.round(lpv || clics) });
    }
...
export async function getAdStatuses(accountId) {
  ...
  let url = `${GRAPH}/${V}/act_${accountId}/ads?fields=${fields}&limit=500`;
  const map = {};
...
export async function getAdsetBudgets(accountId) {
  ...
  let url = `${GRAPH}/${V}/act_${accountId}/adsets?fields=${fields}&limit=200`;
  const map = {};
...
export async function getAds(accountId, datePreset = "last_30d", range = null) {
  ...
  let url = `${GRAPH}/${V}/act_${accountId}/insights?level=ad&fields=${fields}&${period}&limit=500`;
  const data = [];
  for (let i = 0; i < 20 && url; i++) {
    const r = await graphFetch(url);
    const j = await r.json();
...
export async function getAdsetTargeting(accountId) {
  // promoted_object: el EVENTO al que optimiza el adset (PURCHASE vs ADD_TO_CART/INITIATED_CHECKOUT)
  // — con optimization_goal alcanza para mensajes, pero para el techo del rol de embudo hace falta
  // distinguir a qué evento de conversión apunta (goalEmbudo en lib/nomenclatura.js).
  const fields = "id,name,targeting,optimization_goal,destination_type,promoted_object";
  let url = `${GRAPH}/${V}/act_${accountId}/adsets?fields=${fields}&limit=200`;
  const map = {};
  for (let i = 0; i < 15 && url; i++) {
    const r = await graphFetch(url);
...
export async function getTrackingHealth(accountId) {
  ...
  const url = `${GRAPH}/${V}/act_${accountId}/adspixels?fields=${fields}`;
  const r = await graphFetch(url);
  const j = await r.json();
  ...
  const pixels = (j.data || []).map((p) => {
    ...
    const rec = {
      ...
      adsets_activos: 0, // se completa con el cruce de abajo
    };
    pixById[p.id] = rec;
    return rec;
  });

  // Cruce: qué pixel optimizan los conjuntos ACTIVOS (promoted_object.pixel_id). Convierte la
  // bandera "hay un pixel muerto" en un veredicto: ¿algún anuncio activo lo usa (crítico) o es solo
  // un pixel viejo sin uso (limpieza)? Solo ads_read. Si falla, degradamos sin el cruce.
  let pixel_muerto_en_uso = 0, conjuntos_en_pixel_muerto = 0, cruce_ok = false;
  try {
    let url2 = `${GRAPH}/${V}/act_${accountId}/adsets?fields=effective_status,promoted_object&limit=500`;
    for (let i = 0; i < 10 && url2; i++) {
      const rr = await graphFetch(url2);
      const jj = await rr.json();
      if (jj.error) throw new Error(jj.error.message);
      for (const a of jj.data || []) {
        if (a.effective_status !== "ACTIVE") continue;
        const pid = a.promoted_object && a.promoted_object.pixel_id;
        if (pid && pixById[pid]) pixById[pid].adsets_activos += 1;
      }
      url2 = jj.paging && jj.paging.next ? jj.paging.next : null;
    }
    cruce_ok = true;
    for (const p of pixels) if (!p.disparando && p.adsets_activos > 0) { pixel_muerto_en_uso += 1; conjuntos_en_pixel_muerto += p.adsets_activos; }
  } catch { cruce_ok = false; } // sin el cruce, el cerebro igual sabe que hay un pixel muerto
  ...
  return {
    ...
    // veredicto del cruce: un pixel muerto USADO por conjuntos activos = conversiones sin trackear
    cruce_adsets_ok: cruce_ok,
    pixel_muerto_en_uso,            // cuántos pixels muertos los usa algún conjunto activo (crítico si >0)
    conjuntos_en_pixel_muerto,      // cuántos conjuntos activos optimizan sobre un pixel muerto
    pixels: pixels.map(({ id, ...rest }) => rest), // no exponemos el id crudo al snapshot
    emq_capi_disponible: false, // fase 2B: requiere permiso del system user sobre el dataset
  };
}

// URL para VER el anuncio (video incluido) en una pestaña nueva. La fuente del archivo
// (`/{video_id}?fields=source`) NO está permitida para el token de Sistema (#10), así que usamos
// la VISTA PREVIA oficial (`/{ad_id}/previews`): devuelve un iframe cuyo src es una página de
// business.facebook.com que renderiza el anuncio tal cual se sirve, sin login (verificado sep 2026;
// el link vence a las ~24 h, por eso se resuelve al hacer clic y no se guarda). Fallback: el post
// (`effective_object_story_id` → facebook.com/<id>, visible para admins de la página) y, último,
// el anuncio en el Administrador de anuncios.
// Formatos de vista previa en orden de preferencia. NO todos renderizan para todo creativo: los
// de tipo SHARE (ej. MoraShop) fallan en MOBILE_FEED_STANDARD con "La historia de este anuncio no
// está disponible para obtener una vista previa" pero andan en DESKTOP_FEED_STANDARD y en Reels
// (verificado sep 2026). Por eso se prueba cada formato y se VERIFICA el HTML antes de redirigir.
const PREVIEW_FORMATS = ["MOBILE_FEED_STANDARD", "FACEBOOK_REELS_MOBILE", "DESKTOP_FEED_STANDARD", "INSTAGRAM_STORY"];
const PREVIEW_ERROR = /no est[aá] disponible la historia|historia de este anuncio no est[aá] disponible|story of this ad is not available|is not available for preview/i;
export async function getAdPreviewUrl(accountId, adId) {
  const q = (p) => `${GRAPH}/${V}/${p}`; // el token va por header (graphFetch), nunca en la URL
  const manager = `https://www.facebook.com/adsmanager/manage/ads?act=${accountId}&selected_ad_ids=${adId}`;
  for (const fmt of PREVIEW_FORMATS) {
    try {
      const pv = await (await graphFetch(q(`${adId}/previews?ad_format=${fmt}`))).json();
      const body = pv.data && pv.data[0] && pv.data[0].body;
      const src = body && (body.match(/src="([^"]+)"/) || [])[1];
      if (!src) continue;
      const url = src.replace(/&amp;/g, "&");
      // la página de preview responde 200 aunque no pueda renderizar: hay que mirar el HTML.
      // fetch pelado a propósito: es business.facebook.com, NO Graph → no le mandamos el token
      const html = await (await fetch(url, { cache: "no-store", signal: AbortSignal.timeout(8000) })).text();
      if (PREVIEW_ERROR.test(html)) continue;
      return { url, tipo: "preview", formato: fmt };
    } catch { /* probamos el siguiente formato */ }
  }
  try {
    const ad = await (await graphFetch(q(`${adId}?fields=creative{effective_object_story_id}`))).json();
    const story = ad.creative && ad.creative.effective_object_story_id;
    if (story) return { url: `https://www.facebook.com/${story}`, tipo: "post" };
  } catch { /* último recurso */ }
  return { url: manager, tipo: "manager" };
}
