...
export default function App() {
  ...
  // Inversión MANUAL de TikTok (USD) mientras no haya acceso a su API: se carga a mano por tienda,
  // se convierte al dólar oficial de hoy y se SUMA solo a la banda (inversión, MER, CAC, margen).
  // Si algún día se conecta una cuenta tt: real, el botón se esconde para no contar doble.
  const [ttUsd, setTtUsd] = useState("");
  const [ttUsdCmp, setTtUsdCmp] = useState(""); // ídem para el período COMPARADO (así los deltas comparan justo)
  const [ttOn, setTtOn] = useState(false);
  useEffect(() => { if (!tnStore) return; try { setTtUsd(localStorage.getItem("nusa_tt_usd_" + tnStore) || ""); setTtUsdCmp(localStorage.getItem("nusa_tt_usd_cmp_" + tnStore) || ""); setTtOn(localStorage.getItem("nusa_tt_on_" + tnStore) === "1"); } catch {} }, [tnStore]);
  const setTtUsdP = (v) => { setTtUsd(v); try { localStorage.setItem("nusa_tt_usd_" + tnStore, v); } catch {} };
  const setTtUsdCmpP = (v) => { setTtUsdCmp(v); try { localStorage.setItem("nusa_tt_usd_cmp_" + tnStore, v); } catch {} };
  const setTtOnP = (v) => { setTtOn(v); try { localStorage.setItem("nusa_tt_on_" + tnStore, v ? "1" : ""); } catch {} };
  const ttHasReal = allIds.some((id) => String(id).startsWith("tt:"));
  const ttUsdN = parseFloat(String(ttUsd).replace(",", "."));
            ...
            <div className="crow">
              ...
              {tnStore && !ttHasReal && (
                <span className="ttinline" title="Sin acceso a la API de TikTok todavía: cargá a mano lo invertido en ESTE período (en USD). Se convierte al dólar oficial de hoy y se suma a inversión, MER, CAC y margen de la banda de Tienda Nube.">
                  ...
                  {ttOn && <>USD <input className="ttusdh" type="number" min="0" placeholder="0" value={ttUsd} onChange={(e) => setTtUsdP(e.target.value)} /><em>{fx && fx.rate ? (ttExtra > 0 ? "≈ " + money(ttExtra) : "") : "⚠ sin cotización"}</em></>}
                </span>
              )}
            ...
            {account && (
              <div className="crow">
                ...
                {cmpOn && (<>
                  ...
                  {tnStore && !ttHasReal && ttOn && (
                    <span className="ttinline" title="Lo invertido en TikTok en el período COMPARADO (en USD), para que los deltas de la banda comparen justo.">
                      🎵 USD <input className="ttusdh" type="number" min="0" placeholder="0" value={ttUsdCmp} onChange={(e) => setTtUsdCmpP(e.target.value)} /><em>{ttExtraCmp > 0 ? "≈ " + money(ttExtraCmp) : ""}</em>
                    </span>
                  )}
              ...
              <div className="tnstats">
                ...
                <div className="tnstat" title="Margen de contribución del período: facturación × tu margen bruto (producto − costo, antes de la pauta) − inversión en pauta. Lo que queda para cubrir fijos y ganar.">
                  ...
                  {(() => {
                    ...
                    const cmPrev = cm != null && cS ? Math.round(cS.facturacion * (m / 100) - invPrev) : null;
                    return <>
...
const etapaDe = (r) => { const rol = rolEmbudo(r); return rol === "remate" ? "caliente" : rol || "nd"; };
...
function Angulos({ withV, u, modo = "ventas", account, extras = [], tab, accountName = "", preset, cSince, cUntil, onBrief }) {
  ...
  const rowsSheet = useMemo(() => withV.filter((r) => r.sheet), [withV]);
  ...
  const filas = useMemo(() => rowsSheet.map((r) => { const c = niv[fpOf(r)] || null; return { ...r, fp: fpOf(r), c, nivel: c && c.nivel ? c.nivel : "nd", etapa: etapaDe(r), hook: hookRate(r), hold: holdRate(r) }; }), [rowsSheet, niv]);
  ...
  const medHook = useMemo(() => mediana(filas.filter((r) => r.spend >= u.pisoSpend).map((r) => r.hook)), [filas, u.pisoSpend]);
  const celda = (nivel, etapa) => {
    const rs = filas.filter((r) => r.nivel === nivel && (etapa === "total" || r.etapa === etapa));
    const spend = rs.reduce((s, r) => s + r.spend, 0);
    const rev = rs.reduce((s, r) => s + r.spend * (r.roas || 0), 0);
    const ventas = rs.reduce((s, r) => s + (r.ventas || 0), 0);
    const conv = rs.reduce((s, r) => s + (r.conversaciones || 0), 0);
    const roas = spend ? rev / spend : 0, cpa = ventas ? spend / ventas : null, costoConv = conv ? spend / conv : null;
    const hook = mediana(rs.map((r) => r.hook)), hold = mediana(rs.map((r) => r.hold));
    let estado;
    if (!rs.length) estado = "sin_probar";
    else if (spend < u.pisoSpend || rs.length < 2 || nivel === "nd" || etapa === "total") estado = rs.length && nivel !== "nd" && etapa === "total" ? "neutra" : (nivel === "nd" ? "neutra" : "sin_data");
    else if (etapa === "frio") estado = medHook == null || hook == null ? "neutra" : hook >= medHook * 1.15 ? "ganadora" : hook <= medHook * 0.85 ? "perdedora" : "neutra"; // frío: SOLO hook rate
    ...
    return { nivel, etapa, rows: rs, n: rs.length, spend, pct: totalSpend ? spend / totalSpend : 0, roas, cpa, costoConv, hook, hold, estado };
  };
  ...
  const lectura = useMemo(() => {
    ...
    const nd = matriz.nd.total.n; if (nd) out.push(`${nd} creativo${nd > 1 ? "s" : ""} sin clasificar`);
  ...
  const sugerir = async () => {
    ...
    const body = { account, extras, tab, modo, accountName, marca: modaSheet("marca"), publico: modaSheet("publico"), periodo: dias ? dias + " días" : preset, umbral: u, receta, lectura,
      ...
      referencias: { mediana_hook_rate: medHook != null ? +(medHook * 100).toFixed(1) : null, mediana_hold_rate: (() => { const h = mediana(filas.map((r) => r.hold)); return h != null ? +(h * 100).toFixed(1) : null; })(), ...(msg ? { mediana_costo_conv: medVenta && Math.round(medVenta) } : { mediana_roas: medVenta }) } };
    try {
      ...
      <section className="sect">
        ...
        {celdaSel && (
          <div className="cdetail">
            ...
            {[...celdaSel.rows].sort((a, b) => b.spend - a.spend).map((r) => (
              <div className="cdrow" key={r.id}>
                <span className="cdname">{r.nombre} <TF r={r} /><Paused r={r} /></span>
                <span className="cdniv">nivel <select value={r.c && r.c.fuente === "override" ? r.nivel : ""} onChange={(e) => setOverride(r.fp, e.target.value)} title="Override manual del nivel (se guarda para esta planilla)"><option value="">{r.nivel} · {r.c ? r.c.fuente : "nd"}</option>{[5, 4, 3, 2, 1].map((n) => <option key={n} value={n}>{n} · manual</option>)}</select></span>
                <span className="cdmot">{r.c && r.c.motivador ? <><i>{r.c.motivadorTipo}</i> {r.c.motivador}</> : "—"}</span>
                <span className="cdmeta mono">{short(r.spend)} · {msg ? (r.conversaciones ? money(r.costoConv) + "/conv" : "0 conv") : r.roas.toFixed(1) + "x"} · hook {pctf(r.hook)} · hold {pctf(r.hold, 0)}</span>
              </div>))}
          </div>
        )}
      </section>
...
.topinner{display:flex;justify-content:space-between;align-items:center;gap:24px;padding:16px 20px;color:var(--paper);}
.brand{display:flex;gap:15px;align-items:center;flex-wrap:wrap;flex:0 0 auto;}
...
.ttinline{display:inline-flex;align-items:center;gap:6px;font-family:'Space Mono',monospace;font-size:11px;color:var(--ink);margin:4px 0;white-space:nowrap;flex:0 0 auto;}
.ttinline em{font-style:normal;color:#6B6455;}
.ttbtn{display:inline-flex;align-items:center;gap:5px;font-family:'Space Mono',monospace;font-size:12px;font-weight:700;border:2px solid var(--ink);border-radius:6px;padding:5px 9px;background:var(--paper);color:var(--ink);cursor:pointer;user-select:none;}
.ttbtn.on{background:var(--ink);color:var(--paper);}
